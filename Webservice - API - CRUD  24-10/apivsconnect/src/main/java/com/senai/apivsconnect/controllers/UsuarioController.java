...
public class UsuarioController {
    ...
    public ResponseEntity<Object> exibirUsuario(@PathVariable(value = "idUsuario") UUID id){
        ...
        if (usuarioBuscado.isEmpty()) {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Usuario nao encontrado"); // retorno erro 404 ou seja usuario nao encontrado
        }
    ...
    public ResponseEntity<Object> cadastrarUsuario(@RequestBody @Valid UsuarioDto usuarioDto){
        if (usuarioRepository.findByEmail(usuarioDto.email()) != null){
            // Nao pode cadastrar
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Esse email ja esta cadastrado!");//BAD_REQUEST erro 400
        }
